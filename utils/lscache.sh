...
echo "System {"
for cpu in $(find "${ROOTDIR}" -mindepth 1 -maxdepth 1 -name "cpu*" -not -name "cpufreq" -not -name "cpuidle" | sort ) ; do
    CPUCACHEDIR="${cpu}/cache"
    echo "    $(basename ${cpu}) {"
    for index in $(find "${CPUCACHEDIR}" -mindepth 1 -maxdepth 1 -name "index*" | sort ) ; do
